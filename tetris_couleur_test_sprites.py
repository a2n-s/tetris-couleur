...
def afficherGrille(grille : list, alignements : list) -> None:
    ...
    for x in range(largeur):
        for y in range(hauteur - 1, -1, -1):
            ...
            if grille[x][y] != VIDE:
                if (x, y) == (0, 0):
                    brique = [int(grille[x][y] != grille[x + 1][y]), int(grille[x][y] != grille[x][y + 1]), 1, 1]
                elif (x, y) == (0, hauteur - 1):
                    brique = [int(grille[x][y] != grille[x + 1][y]), 1, 1, int(grille[x][y] != grille[x][y - 1])]
                elif (x, y) == (largeur - 1, 0):
                    brique = [1, int(grille[x][y] != grille[x][y + 1]), int(grille[x][y] != grille[x - 1][y]), 1]
                elif (x, y) == (largeur - 1, hauteur - 1):
                    brique = [1, 1, int(grille[x][y] != grille[x - 1][y]), int(grille[x][y] != grille[x][y - 1])]
                elif x == 0:
                    brique = [int(grille[x][y] != grille[x + 1][y]), int(grille[x][y] != grille[x][y + 1]), 1, int(grille[x][y] != grille[x][y - 1])]
                elif x == largeur - 1:
                    brique = [1, int(grille[x][y] != grille[x][y + 1]), int(grille[x][y] != grille[x - 1][y]), int(grille[x][y] != grille[x][y - 1])]
                elif y == 0:
                    brique = [int(grille[x][y] != grille[x + 1][y]), int(grille[x][y] != grille[x][y + 1]), int(grille[x][y] != grille[x - 1][y]), 1]
                elif y == hauteur - 1:
                    brique = [int(grille[x][y] != grille[x + 1][y]), 1, int(grille[x][y] != grille[x - 1][y]), int(grille[x][y] != grille[x][y - 1])]
                else:
                    brique = [int(grille[x][y] != grille[x + 1][y]), int(grille[x][y] != grille[x][y + 1]), int(grille[x][y] != grille[x - 1][y]), int(grille[x][y] != grille[x][y - 1])]
                
                BRIQUES[grille[x][y]][bin_vers_dec_4(brique[0], brique[1], brique[2], brique[3])].blit(INFO.SCREEN, (RATIO*x, RATIO*(hauteur - 1 - y)))
            else:
                fill(BLACK)
                stroke(DARK_GRAY)
                strokeWeight(1)
                rectangle((RATIO*x, RATIO*(hauteur - 1 - y)), RATIO, RATIO)
       
def bin_vers_dec_4(a : [0, 1], b : [0, 1], c : [0, 1], d : [0, 1]) -> int:
    return a*2**3 + b*2**2 + c*2 + d
       
def chargerBriques() -> list:
    briques = []
...
def chargerCreaturesConnues(nameOfFile = "creatures.txt") -> list:
    ...
    for i in range(n):
        ...
        # les points faibles mis en exergue
        creatures[i][2] = creatures[i][2].split(":")
        for j in range(len(creatures[i][2])):
            creatures[i][2][j] = creatures[i][2][j].split(".")
            # print(creatures[i][2][j])
            if creatures[i][2][j] != ['']:
                for l in range(len(creatures[i][2][j])):
                    creatures[i][2][j][l] = int(creatures[i][2][j][l])
...
def trierCreature(creaturesConnues : list) -> list:
...
def gererNouvellesCreatures(creatures : list, creaturesCapturees : list) -> list:
    def lesMemesCreatures(creatureA, creatureB : list) -> bool:
        lA = len(creatureA)
        hA = len(creatureA[0])
        lB = len(creatureB)
        hB = len(creatureB[0])
    ...
    for (creatureB, i_couleur, pointsFaibles) in creaturesCapturees:
        connue = False
        for A in range(len(creatures)):
            if lesMemesCreatures(creatures[A][0], creatureB):
                ...
                break
        if not connue: 
            creatures.append([creatureB, [1 if i == i_couleur else 0 for i in range(len(ETATS_BRIQUES))], pointsFaibles]) 
            print("Créature inconnue capturée : ")
            for y in range(len(creaturesConnues[-1][0][0])):
                line = ""
                for x in range(len(creaturesConnues[-1][0])):
                    line += str(creaturesConnues[-1][0][x][y])
                print("\t" + line)
            
        
    return trierCreature(creatures)
    
def montrerCreature(x : int, y : int, c : int, creature : list) -> None:
    l = len(creature[0])
    h = len(creature[0][0])
    d = max(l, h)
    s = c / d
...
def sauvegarderCreatures(creaturesConnues : list, nameOfFile = "creatures.txt") -> None:
    ...
    for creature in creaturesConnues:
        ...
        f.write("|")
        # les points faibles rencontrés
        for i in range(len(creature[2])):
            for j in range(len(creature[2][i])):
                f.write(str(creature[2][i][j]))
                if j != len(creature[2][i]) - 1: f.write(".")
            if i != len(creature[2]) - 1: f.write(":") 
        f.write("\n")
    f.close()
    return None
# # #
...
def setup():
    ...
    ### INITIALISATION
    global grille, pas, etat_jeu, etat_jeu_precedent, x, y, score, frames, temps, alignements, combo, creaturesConnues, creatureSelectionnee, point, instants_de_jeu, BRIQUES
    commencerUnePartie()
    alignements = []
...
def draw():
    ...
    ### LE BESTIAIRE
        ...
        if creaturesConnues != []:
            montrerCreature(INFO.w/2, INFO.h/2 - 100, 200, creaturesConnues[creatureSelectionnee])
    ...
    # # #
        ...
        afficherGrille(grille, alignements)
        SPEED = f(temps)
        ...
        # # #
            ...
            ### GESTION DE LA DISPARITION DES BLOCS GAGNANTS ET CALCULS DES SCORES
                ...
                if pas == 0:
                    ...
                    creaturesConnues = gererNouvellesCreatures(creaturesConnues, creaturesCapturees)
                    point = 0
...
# TODO 1 : définir un "score total de capture" pour savoir qui est le meilleur a la capture de créatures !
